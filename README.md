# Style Scratcher (스타일 스크래처) v2.1.0
...
## 📌 Style Scratcher v2.1.0 소개

기존 브라우저 개발자 도구(DevTools)의 수많은 상속 스타일과 난해한 난독화 클래스(`.css-x98k1`)에 지치셨나요?

**Style Scratcher**는 웹 브라우저 화면을 **피그마(Figma) 캔버스**처럼 자유롭게 다룰 수 있게 해주는 고성능 화이트 미니멀 Chrome 브라우저 확장 프로그램(Manifest V3)입니다.

### 🏷️ 버전 관리 체계 (Semantic Versioning)
Style Scratcher는 표준 3자리 버전 체계(`Major.Minor.Patch`)를 준수합니다.
- **Major (첫 번째 자리)**: 아키텍처 및 핵심 패러다임이 바뀌는 대규모 변화
- **Minor (두 번째 자리)**: 핵심 신규 기능 및 주요 사용자 경험 확장
- **Patch (세 번째 자리)**: 세부 버그 픽스 및 미세 성능 최적화

---

## ✨ v2.1.0 핵심 기능 (Key Features)
...
- **쾌속 원복 줌아웃**: 손을 떼는 순간 줌인보다 2.6배 빠른 스내피 줌아웃(200ms 곡선)으로 원래 배율로 순식간에 복귀합니다.
- **브라우저 노멀 줌과의 차별화**: 브라우저의 전체 레이아웃을 깨뜨리는 <kbd>Ctrl</kbd> + <kbd>+</kbd>/<kbd>-</kbd> 노멀 줌과 달리, 페이지 레이아웃 변형 없이 마우스 위치의 디테일과 픽셀을 순간적으로 확대해 볼 수 있는 캔버스 돋보기 경험을 선사합니다.

### 2. 📐 다중 단위 거리 측정 엔진 (Multi-Unit Measurement)
- **자유로운 단위 스위치**: `px` 뿐만 아니라 타이포그래피/반응형 단위인 `rem`, `em`, `%`, `vw`, `vh`, `pt`로 즉시 환산.
- **스마트 룰러 듀얼 뱃지**: 요소를 클릭해 고정한 뒤 다른 요소 위에 마우스를 올리면 실시간으로 상·하·좌·우 간격이 `1.5rem (24px)` 형태의 듀얼 뱃지로 표시됩니다.
- **박스 모델 단위 연동**: Margin과 Padding의 시각적 박스 모델에서도 선택된 단위로 즉시 변환.

### 3. ⚡ 인터랙션 & JS 이벤트 리스너 탐지기 (Interaction Detector)
...
### 4. 🎮 WebGL & Canvas / SVG 심층 그래픽 분석 (Graphics Telemetry)
...
### 5. 🔓 복사/우클릭 차단 무력화 & 툴팁 프리징 (Power Tools)
...
### 6. 🎬 애니메이션 슬로우모션 & 모션 인스펙터 (Motion Lab)
...
### 7. 🎨 화이트 미니멀 스튜디오 디자인 (White Minimal Studio)
...
1. 본 리포지토리의 [Releases](https://github.com/hslcrb/style-scrather/releases) 페이지에서 최신 `style-scrather-v2.1.0.zip` 파일을 다운로드합니다.
...
| <kbd>Ctrl</kbd> + <kbd>+</kbd> / <kbd>-</kbd> | **브라우저 노멀 줌** (표준 레이아웃 확대/축소) |
...
WebGL 3D 큐브, 인스턴트 줌, 애니메이션 슬로우모션, 복사 차단 무력화 기능을 웹 애플리케이션 형태로 즉시 시연할 수 있습니다.
